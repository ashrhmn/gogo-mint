...
const LayoutMain = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <>
      <nav className="flex justify-between fixed items-center shadow-md shadow-gray-300 top-0 left-0 right-0 px-4 z-50 bg-white h-16">
        <div>Logo</div>
        <div className="flex">
          <Link href={`/`}>
            <a className="p-2 font-bold text-gray-800 hover:text-black">Home</a>
          </Link>
          <Link href={`/dashboard`}>
